...
.get((req, res, next) => {
    Campsite.find()
    //next when we retrieve campsites we will populate the comments subdocument with the author that matches the objectId 
...
.put(authenticate.verifyUser, (req, res, next) => {
        ...
        .then(campsite => {
            if(req.user._id.equals(campsite.comments.id(req.params.commentId).author)) {
                ...
                const err = new Error('You do not have privileges!');
                err.status = 403;
                return next(err);
            }
        })
